...
if [ $regularizer = "GL1" ]; then
    ...
    BASE="python main.py --purpose exp --loss ${loss} --regularizer NatOG --lam_shrink 0.1 --NatOG_grpsize 10 --NatOG_overlap_ratio 0.1 --weight_decay 0.0001 --accuracy -1.0 --max_epochs ${max_epochs} --max_time 43200.0 --save_seq True --compute_optim ${compute_optim} --data_dir ~/db --dataset a9a"    
fi
